...
class DigitalResumeApp {
    ...
    // Wait for resumeData to be available
    async waitForData() {
        let attempts = 0;
        const maxAttempts = 50;
        
        while (typeof window.resumeData === 'undefined' && attempts < maxAttempts) {
            await new Promise(resolve => setTimeout(resolve, 100));
            attempts++;
        }
        
        if (typeof window.resumeData === 'undefined') {
            throw new Error('Resume data failed to load');
        }
    ...
    initTypingAnimation() {
        ...
        if (!this.typingElement || !window.resumeData?.personal?.typingTexts) return;

        const texts = window.resumeData.personal.typingTexts;
        this.typeText(texts);
    }
    ...
    typeText(texts) {
        ...
        const currentText = texts[this.typingTextIndex];
        
        if (this.typingIndex < currentText.length) {
            this.typingElement.textContent = currentText.substring(0, this.typingIndex + 1);
            this.typingIndex++;
            setTimeout(() => this.typeText(texts), 100);
        } else {
            setTimeout(() => this.eraseText(texts), 2000);
        }
    }
    ...
    eraseText(texts) {
        ...
        if (this.typingIndex > 0) {
            this.typingElement.textContent = currentText.substring(0, this.typingIndex - 1);
            this.typingIndex--;
            setTimeout(() => this.eraseText(texts), 50);
        } else {
            this.typingTextIndex = (this.typingTextIndex + 1) % texts.length;
            setTimeout(() => this.typeText(texts), 500);
        }
    }

    // Initialize stats counter animation
    initStatsAnimation() {
        const statsElements = document.querySelectorAll('.stat-number[data-value]');
        if (!statsElements.length || !window.resumeData?.stats) return;

        // Update stats with data from resumeData
        window.resumeData.stats.forEach((stat, index) => {
            const statElement = statsElements[index];
            if (statElement) {
                statElement.dataset.value = stat.value;
                this.animateCounter(statElement, stat.value, stat.suffix || '');
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Ensure data is loaded before initializing app
    if (typeof window.resumeData !== 'undefined') {
        ...
        setTimeout(() => {
            clearInterval(checkData);
            console.error('Failed to load resume data within timeout period');
        }, 10000);
    }
});
